      ...
      <StaggerGroup className="flex flex-col gap-3">
        {awards.items.map((award) => (
          <StaggerItem key={award.awardId}>
            <AwardTile award={award} reduceMotion={reduceMotion} />
          </StaggerItem>
        ))}
      </StaggerGroup>
...
function AwardTile({
  ...
  return (
    ...
    >
      <div className="jutge-panel-body flex items-start gap-3 py-3 sm:gap-4 sm:py-4">
        <AwardIcon award={award} className="h-12 w-12 shrink-0 sm:h-14 sm:w-14" />

        <div className="min-w-0 flex-1">
          <p className="text-jutge-muted text-[10px] font-bold uppercase sm:text-xs">
            {award.timeLabel}
          </p>
          {award.problemId && (
            <a
              ...
              className="text-jutge-blue mt-0.5 inline-block text-xs font-semibold no-underline hover:underline sm:text-sm"
            >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="jutge-score text-jutge-blue mt-0.5 block truncate text-base leading-tight font-semibold no-underline hover:underline sm:text-lg"
          >
            {award.title}
          </a>
          {award.info && (
            <p className="text-jutge-muted mt-1 line-clamp-2 text-xs leading-relaxed sm:text-sm">
              {award.info}
            </p>
          )}
        </div>
        ...
        {award.youtube && (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-jutge-blue flex shrink-0 flex-col items-center gap-1 self-center px-1 text-center text-[10px] font-semibold no-underline hover:underline sm:text-xs"
          >
            <Youtube className="h-5 w-5 shrink-0 sm:h-6 sm:w-6" />
            <span className="hidden max-w-16 leading-tight sm:inline">
